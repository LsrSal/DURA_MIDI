...
## V4:
- Any MOTU or OPCODE is working now. (I believe, and you test.) Added settings to enable/disable 2 layers of Running Status.  "F0 7D 00 F7" to read and "F0 70 nn F7" to set (will also return status read). On cable_8 only! Valid values are 0-3, only 2 LSBs to control Running Status for "cable" (port) number and for standard MIDI Running Status. Pretty much any MOTU and Opcode Midi interface will work with setting "02". Original MTP will gain a bit more bandwidth with settings "03". Setting saved to flash and supposed to survive reboots. "00" will not apply Running Status at all. Most compatible but cost some bandwidth, if you ever manage to saturate your MIDI traffic.
 ______________________________________________
### Some testing:
