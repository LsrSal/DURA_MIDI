...
## V4:
- Any MOTU or OPCODE is working now. (I believe, and you test.) Added settings to enable/disable 2 layers of Running Status.  "F0 7D 00 F7" to read and "F0 70 nn F7" to set (will also return status read). On cable_8 only! Valid values are 0-3, only 2 LSBs to control Running Status for "cable" (port) number and for standard MIDI Running Status. Pretty much any MOTU and Opcode Midi interface will work with setting "02" (Cable's only Running Status). Original MTP will gain a bit more bandwidth with settings "03" (Cable AND MIDI Running Status). Setting saved to flash and supposed to survive reboots. "00" will not apply Running Status at all. Most compatible but cost some bandwidth, if you ever manage to saturate your MIDI traffic.

## V5:
- Added serial speed selection: third bit of the same variable as above (that is +4 decimal). Interface speed is "Fast" (125Kbaud) when set to zero, and "1MHz" (31.25Kbaud) when set to 1. May need to downgrade speed for some older interfaces. Tested with MTPs but make no sense to set it slow. Maybe needed for some other models without Fast Mode.
- For most folks safest default is all x3 LSBs to be zeros, most compatible - Fastest serial, no Running Status. Send SysEx "F0 70 00 F7" to cable 8. Then if needed to change - edit that "00" in the message. For MTP original (no display) the best settings is "03", and for the rest of MTPs (with LCD) - "02" (reduction of cable traffic with Running Status), or "00" if some of your peripheral MIDI devices do not support Running Status (though it will be against MIDI specs, but apparently still happens). MTP_AV has settings for own Running Status per cable. The difference is that above Running Status will reduce traffic over serial bottleneck, and settings in the MTP will reduce message only over Each MIDI cable while serial interface still have to pass whole message. Note that MIDI over USB cannot have Running Status, USB MIDI message has mandatory length and cannot be reduced with Running Status. Applying Running Status can be done only outside of USB link.
- Added power and activity indicator RGB LED WS2812 on GPIO16. Already present on RP2040_Zerio board, can be easily added to Pico and others.
 ______________________________________________
### Some testing:
